...
class TacoTrainingHelper(Helper):
    def __init__(self, batch_size, targets, hparams, gta, evaluating, global_step):
        ...
        with tf.name_scope('TacoTrainingHelper'):
            ...
            self._output_dim = hparams.num_mels
            self._reduction_factor = hparams.outputs_per_step
            self._ratio = tf.convert_to_tensor(hparams.teacher_forcing_ratio)
            self.gta = gta
    ...
    def initialize(self, name=None):
        ...
        if self.gta:
            ...
            if self._hparams.teacher_forcing_mode == 'scheduled':
                self._ratio = _teacher_forcing_ratio_decay(self._hparams.teacher_forcing_init_ratio,
...
def _teacher_forcing_ratio_decay(init_tfr, global_step, hparams):
    ...
    if hparams.teacher_forcing_final_ratio is not None:
        alpha = float(hparams.teacher_forcing_final_ratio / hparams.teacher_forcing_init_ratio)

    else:
        assert hparams.teacher_forcing_decay_alpha is not None
        alpha = hparams.teacher_forcing_decay_alpha

    # Compute natural cosine decay
    tfr = tf.train.cosine_decay(init_tfr,
                                global_step=global_step - hparams.teacher_forcing_start_decay,
                                # tfr ~= init at step 10k
                                decay_steps=hparams.teacher_forcing_decay_steps,
    ...
    narrow_tfr = tf.cond(
        tf.less(global_step, tf.convert_to_tensor(hparams.teacher_forcing_start_decay)),
